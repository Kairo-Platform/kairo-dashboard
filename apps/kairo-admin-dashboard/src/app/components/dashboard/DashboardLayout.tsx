...
import { auth, setAuthUser } from "@/app/store/auth";
...
import { getApiData } from "@/app/lib/utils/getApiData";
...
export const DashboardLayout: FC<DashboardLayoutProps> = ({
  ...
  const fetchLoggedInUser = () => {
    ...
    return fetchAuthUser()
      .then((response) => {
        const user = getApiData<{ type?: string }>(response);
        if (user?.type === USER_TYPE.MERCHANT) {
          router.replace(URL.MERCHANT_DASHBOARD_URL);
        }
        if (user?.type === USER_TYPE.AGENCY) {
          router.replace(URL.AGENCY_DASHBOARD_URL);
        }
        if (user?.type === USER_TYPE.BUSINESS_AGGREGATOR) {
      ...
      .catch((error: { statusCode?: number }) => {
        if (AuthUtils.getUserType() === USER_TYPE.STAFF || isAdminStaff) {
